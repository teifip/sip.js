...
function stringify(m, display = false) {
  var s;
  if(m.status) {
  ...
  if (!m.content) {
    return display ? s : Buffer.from(s);

  } else if (m.content instanceof Buffer) {
    return display ? s + m.content.toString('hex')
                   : Buffer.concat([Buffer.from(s), m.content]);
  } else {
    return display ? s + m.content
                   : Buffer.concat([Buffer.from(s), Buffer.from(m.content)]);
  }
}
...
function makeStreamParser(onMessage, binaryTypes = []) {
  var m;
  var r = Buffer.alloc(0);

  function headers(data) {
    r = Buffer.concat([r, data]);
    var idx = r.indexOf(Buffer.from('\r\n\r\n'));

    if(idx !== -1) {
      m = parse(r.slice(0, idx).toString());
      r = r.slice(idx + 4);
  ...
  function content(data) {
    ...
    if(r.length >= m.headers['content-length']) {
      m.content = r.slice(0, m.headers['content-length']);

      if (!binaryTypes.includes(m.headers['content-type'])) {
        m.content = m.content.toString();
      }

      onMessage(m);

      var s = r.slice(m.headers['content-length']);
      state = headers;
      r = Buffer.alloc(0);
      headers(s);
    }
...
function parseMessage(s, binaryTypes = []) {
  var idx = s.indexOf(Buffer.from('\r\n\r\n'));
  var m = idx !== -1 ? parse(s.slice(0, idx).toString()) : null;

  if (m) {
    m.content = s.slice(idx + 4);
    var cl = Number(m.headers['content-length']);
    if (cl > m.content.lenght) {
      m.content = m.content.slice(0, cl);
    }
...
function makeStreamTransport(protocol, connect, createServer, binaryTypes, callback) {
...
function makeWsTransport(options, callback) {
  ...
  function init(ws) {
    ...
    ws.on('message', function(data) {
      var msg = parseMessage(data, options.binaryTypes);
      if(msg) {
  ...
  function makeClient(uri) {
    ...
    function send_connecting(m) { queue.push(m); }
    function send_open(m) { socket.send(stringify(m)); }
...
function makeUdpTransport(options, callback) {
  ...
  function open(remote, error) {
    return {
      send: function(m) {
        socket.send(stringify(m), remote.port, remote.address);
      },
      protocol: 'UDP',
      release : function() {}
    };
...
exports.create = function(options, callback) {
  if (!Array.isArray(options.binaryTypes)) options.binaryTypes = [];
  ...
  function decodeFlowToken(token) {
    var s = Buffer.from(token, 'base64').toString('ascii').split(',');
